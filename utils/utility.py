...
def save_mask(pred_masks, save_base_path, category_list, video_name_list):
    ...
    for idx in range(bs):
        category, video_name = category_list[0], video_name_list[0]
...
def save_list(miou_list, f_score_list, path):
    filename = open(path, 'w')
    for value1, value2 in zip(miou_list, f_score_list):
        filename.write(f"{value1},{value2}\n")
    filename.close()
